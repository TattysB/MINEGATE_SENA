...
def api_solicitudes_coordinacion(request):
    ...
    if tipo == "internas":
            ...
            visitas_data.append(
                {
                    ...
                    "fecha_solicitud_orden": (
                        visita.fecha_solicitud.isoformat()
        ...
        visitas_ext = VisitaExterna.objects.filter(
            estado="enviada_coordinacion"
        ).order_by("-fecha_solicitud", "-id")
        if buscar:
...
def api_accion_coordinacion(request, tipo, visita_id, accion):
    if accion == "detalle" and request.method == "GET":
        if tipo == "interna":
            ...
            detalle = {
                ...
                "responsable": visita.responsable,
                "tipo_documento": (
                    visita.get_tipo_documento_responsable_display()
                    if visita.tipo_documento_responsable
                    else "N/A"
                ),
                ...
                "hora_fin": (
                    visita.hora_fin.strftime("%I:%M %p") if visita.hora_fin else "N/A"
                ),
    ...
    if accion == "observaciones":
        if tipo == "interna":
            ...
            visita = get_object_or_404(VisitaExterna, pk=visita_id)
            campo_observaciones = "observacion"

        if request.method == "GET":
            return JsonResponse(
                {
                    "success": True,
                    "observaciones": getattr(visita, campo_observaciones, "") or "",
                }
            )
        ...
        setattr(visita, campo_observaciones, observaciones)
        visita.save(update_fields=[campo_observaciones])
        ...
        if tipo == "interna":
            HistorialAccionVisitaInterna.objects.create(
                ...
                descripcion=f"Observaciones actualizadas por coordinación ({request.user.username}).",
        ...
        return JsonResponse(
            {"success": True, "message": "Observaciones guardadas correctamente."}
        )
    ...
    if accion == "rechazar":
        ...
        if getattr(visita, "fecha_visita", None) and getattr(
            visita, "hora_inicio", None
        ):
            fecha_anterior = timezone.datetime.combine(
                visita.fecha_visita, visita.hora_inicio
            )
        ...
        return JsonResponse(
            {
                "success": True,
                "message": "🔄 Solicitud enviada a reprogramación. El instructor deberá elegir una nueva fecha.",
            }
        )
...
def registro_coordinacion(request):
    ...
    return render(
        request, "coordinador/registro_coordinacion.html", {"registros": registros}
    )
